//#1
//Input is a positive number n. Find product of natural numbers multiples to 3 and below n
...
double ys = Convert.ToDouble(Console.ReadLine());

if(xs is >= -1 and <= 3 && ys is <= 4 and >= -2)
{
...
if (((bt - 2) / 2.5f) <= at && ((bt - 2) / -2.5f) >= at && bt is >= -3 and <= 2)
{
    Console.WriteLine("Point is INSIDE area");
}
else{Console.WriteLine("Point is OUTSIDE area");}
